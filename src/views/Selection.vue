...
<template>
    <ChooseImage 
        ...
        :fromGallery="store.wasPhotoSelected"
        @selectionUpdate="store.wasPhotoSelected = $event"
        :images="photos" 
        :image_path="photo_path" 
        :max_image_size="max_image_size" 
        :image_data="store.photo_data"
        @image_chosen="store.photo_data = $event; nextStep();"></ChooseImage>

    <ChooseImage 
        v-else-if="step == 2"
        Message = "Select/Upload Artwork"
        :fromGallery="store.wasArtSelected"
        @selectionUpdate="store.wasArtSelected = $event"
        :images="paintings" 
        :image_path="art_path" 
        :max_image_size="max_image_size" 
        :image_data="store.art_data"
        @image_chosen="store.art_data = $event; nextStep();"></ChooseImage>
...
export default {
    ...
    methods: {
        nextStep() {
            if (this.store.art_data == "") {
                this.$router.push("/selection/art")
                this.step = 2
            } else {
                this.step = 3
                this.submit()
        ...
        async submit(prev_tries=0) {
            this.error = false
            this.store.contentDOMID = "LoadingBlur"
