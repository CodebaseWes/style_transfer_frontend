...
export default {
    ...
    props : ["stage"],
    data() {
        /* photos and paintings could be retrieved from an API call
        that executes a Lambda function, but API calls count against my service quota...*/
        return {
            store : useStore(),
            max_image_size : 50000, //The API won't accept images that are too large
            step : 1,
    ...
    methods: {
        async submit(prev_tries=0) {
            ...
            /*We don't want to spawn a new lambda function instance
            if the ping from earlier has not resolved */
            while (!this.store.pingFinished) {
                await waitFor(1000)
            }
            ...
            try {
                let response = await fetch(this.store.apiURL, {
                    ...
                    body: JSON.stringify({
                        "photo" : this.store.photo_data,
                        "art" : this.store.art_data
                    })
                });
            ...
            //if there is an on the first call, try again
            if (prev_tries == 0) {
    ...
    watch: {
        /* If a user clicks the browser back button, make sure that 
        the component updates according to the value of the stage prop
         */
        stage(newValue) {
            this.setStage()
        }
